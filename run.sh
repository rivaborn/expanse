...
usage() {
	cat <<EOF
Usage:
    ./run.sh dev audit          npm audit in backend and frontend
    ./run.sh dev outdated       npm outdated in backend and frontend
	...
	prod)
		case "${2:-}" in
			...
			update)
				./run.sh prod down
				git pull
				docker compose -f "$PROD_COMPOSE" pull --ignore-pull-failures
